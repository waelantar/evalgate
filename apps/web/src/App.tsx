...
export function App() {
  ...
  useEffect(() => {
    if (state.errorMessage !== null) errorRef.current?.focus()
  }, [state.errorMessage])
  ...
  function focusEvidence(evidenceId: string) {
    const target = evidenceRefs.current[evidenceId]
  ...
  return <main>
    ...
    <section className="workbench" aria-labelledby="ask-heading">
      <div className="panel"><p className="eyebrow">Local fixture mode</p><h2 id="ask-heading">Inspect an answer</h2>
        ...
        <p className="status" role="status" aria-live="polite" aria-atomic="true">{labels[state.phase]}</p>{state.errorMessage && <p ref={errorRef} className="error" role="alert" tabIndex={-1}>{state.errorMessage}</p>}
      </div>
      <div className="panel answer-panel"><p className="eyebrow">Answer</p><h2 id="answer-heading">{state.answerStatus === 'insufficient_support' ? 'Insufficient support' : 'Grounded response'}</h2><p className="answer">{contentText(state.answer || (state.phase === 'idle' ? 'Your streamed answer will appear here.' : 'Waiting for answer text...'))}</p>
        {state.citationsReady && state.citations.length > 0 && <div className="citations" aria-labelledby="citations-heading"><h3 id="citations-heading">Citations</h3>{state.citations.map((citation, index) => <button key={`${citation.evidence_id}-${index}`} type="button" className="citation" aria-controls={`evidence-${citation.evidence_id}`} onClick={() => focusEvidence(citation.evidence_id)}>{contentText(citation.title)} · {contentText(citation.section_key)}</button>)}</div>}
      </div>
    </section>
    <section className="panel evidence-panel" aria-labelledby="evidence-heading"><p className="eyebrow">Inspectable sources</p><h2 id="evidence-heading">Evidence</h2>{state.evidence.length === 0 ? <p className="muted">Evidence appears after retrieval completes.</p> : state.evidence.map((item) => <article id={`evidence-${item.evidence_id}`} className="evidence" key={item.evidence_id} ref={(element) => { evidenceRefs.current[item.evidence_id] = element }} tabIndex={-1}><h3>{contentText(item.title)}</h3><p className="muted">{contentText(item.source_key)} · {contentText(item.section_key)} · rank {item.rank}</p><p>{contentText(item.content)}</p><small>Evidence ID: {contentText(item.evidence_id)}</small></article>)}</section>
    <ResultsWorkbench />
  </main>
}
