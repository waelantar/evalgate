...
EvalGate uses a serialized product-version path following Semantic Versioning. Before `1.0.0`, a minor bump introduces a new observable API, CLI, UI, integration, or distributable capability; a patch bump introduces compatible internal foundation, evidence, governance, or hardening. EG-017, EG-018, and EG-019 add the final pre-stable product experience, real-world showcase, and product-shell/onboarding refinement at `0.10.0`, `0.11.0`, and `0.12.0`. EG-014 alone may then establish `1.0.0`, and only after every R3 release gate passes. After `1.0.0`, normal Semantic Versioning applies.
...
When an artifact embeds the product version, run pre-bump implementation checks first, apply the declared version, then build and validate the final version-bearing artifact. Only post-bump evidence is acceptance evidence for that artifact. EG-013D therefore produces the `0.9.0` release candidate; EG-017, EG-018, and EG-019 advance the reviewed pre-stable product to `0.10.0`, `0.11.0`, and `0.12.0`; EG-014 conditionally establishes `1.0.0` and rebuilds/scans the final image from unchanged accepted definitions.
