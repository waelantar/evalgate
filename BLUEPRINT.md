...
EG-001 established the repository, governance, locked toolchain, PostgreSQL Compose definition, health/status foundations, and secret-free CI definition. EG-002 and EG-003 are merged and accepted with provider-neutral ports, explicit fixtures, fail-closed configuration, the verified 384-dimensional reference identity, and the PostgreSQL/pgvector foundation schema. EG-004 is merged and accepted with the original governed corpus, real pinned reference embedding, and idempotent transactional ingestion. EG-005 is merged and accepted with bounded explainable lexical, exact-vector, and hybrid retrieval. EG-006 is merged and accepted with a bounded versioned prompt, explicit answer modes, evidence-ID-only provider output, and server-derived citations. EG-007 is implemented and locally verified on its review branch with the frozen POST/fetch/SSE contract, strict parser, awaited backpressure, typed pre/post-header errors, and cancellation cleanup; it awaits repository-owner review and manual merge. Nothing is tagged, released, or deployed.
